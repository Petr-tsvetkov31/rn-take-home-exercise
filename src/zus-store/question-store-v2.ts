...
// Could support multiple selection in the future (no multiple questions in data)
...
export const useQuestionV2 = create<QuestionStoreState>()((set) => ({
  question: undefined,
  options: undefined,
  status: undefined,
  ...
  onSelectCategory: (index) =>
    set(({ question }) => {
      if (index === -1 || index >= question?.questionData.categories?.length)
        return { status: 'idle' }
  ...
  onCheckAnswer: () =>
    set(({ options, selectedCategoryIndex, question }) => {
      ...
      const { questionType, correctAnswer } = question?.questionData ?? {}
      if (questionType === 'mcq') {
...
export type QuestionStore = ReturnType<typeof useQuestionV2>
